...
class AdminController extends Controller
{
    ...
    public function CampusVistingRequestFunction(Request $request)
    {
        ...
        if (Auth::id()) {
            ...
            $to_date = $request->date_two;

            // Clear applied filters
            if ($request->has('clear')) {
                return redirect()->route('VisitRequest');
            }
            //applying filter between two date
            if ($request->date && $request->date_two){
                $StoreVisitPass = StoreVisitPass::whereBetween('Date', [$from_date, $to_date])->get();
            }
            else{
                $StoreVisitPass = StoreVisitPass::all();
            }

            return view('admin.AdminViewCampusVisitRequest', compact('StoreVisitPass'));
        } else {
            return view('auth.login');
        }
    }
    public function ApproveVisitPassButtonFunction($id){
        if (Auth::id()) {
            $StoreVisitPass = StoreVisitPass::find($id);
            $StoreVisitPass->VisitingStatus = 'Approved';
            $StoreVisitPass->save();
            // $StoreVisitPass = Auth::user()->id;
            // $StoreVisitPass = StoreVisitPass::where('userID', $StoreVisitPass)->get();
            Alert::info('successful', 'approved successfully');
            return redirect()->back();
    }
    else{
        return view('auth.login');
